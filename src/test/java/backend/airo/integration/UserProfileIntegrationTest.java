...
class UserProfileIntegrationTest {
    ...
    void userProfileCrudIntegrationTest() throws Exception {
        ...
        Map<String, Object> dataMap = (Map<String, Object>) responseMap.get("result");
        Long userId = Long.valueOf(dataMap.get("id").toString());

        assertNotNull(userId);
        assertTrue(userId > 0);

        // 2. READ - 개별 회원 프로필 조회
        mockMvc.perform(get("/api/v1/user-profiles/{id}", userId))
                .andDo(print())
                .andExpect(status().isOk())
        ...
        mockMvc.perform(put("/api/v1/user-profiles/{id}", userId)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(updateRequest)))
                .andDo(print())
                .andExpect(status().isOk())
                .andExpect(jsonPath("$.data.id").value(userId))
                .andExpect(jsonPath("$.data.name").value("홍길동수정"))
                .andExpect(jsonPath("$.data.contact").value("010-9999-8888"))
                .andExpect(jsonPath("$.data.email").value("updated@example.com"));

        // 5. DELETE - 회원 프로필 삭제
        mockMvc.perform(delete("/api/v1/user-profiles/{id}", userId))
                .andDo(print())
                .andExpect(status().isOk());
    ...
    void userProfileNotFoundTest() throws Exception {
        ...
        mockMvc.perform(put("/api/v1/user-profiles/{id}", 999L)
                        .contentType(MediaType.APPLICATION_JSON)
                        .content(objectMapper.writeValueAsString(updateRequest)))
                .andDo(print())
                .andExpect(status().is4xxClientError());
        ...
        // 존재하지 않는 ID로 삭제
        mockMvc.perform(delete("/api/v1/user-profiles/{id}", 999L))
                .andDo(print())
                .andExpect(status().is4xxClientError());
    }
}