...
public class CommentController implements CommentControllerSwagger {
    ...
    public Response<CommentCountResponse> getCommandCount(
            @PathVariable Long postId
    ) {
        Long commentCount = commentUseCase.getCommentCount(postId);
        return Response.success(
                new CommentCountResponse(
                        postId,
                        commentCount
                )
        );
    ...
    public Response<List<CommentResponse>> createComment(
            @PathVariable Long postId
    ) {
        List<Comment> commentList = commentUseCase.getCommentList(postId);
        return Response.success(
                commentList.stream().map(list ->
                        new CommentResponse(list.getId(), list.getContent(), list.getPostId(), list.getUserId())
                ).toList()
        );
    }
