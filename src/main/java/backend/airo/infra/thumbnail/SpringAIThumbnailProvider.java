...
public class SpringAIThumbnailProvider implements LLMProvider {
...
//    private final ChatClient ollamaClient;
    ...
    private ThumbnailResult parseResponse(String response, ThumbnailRequest request) {
        try {
            // JSON 부분만 추출 (LLM 응답에서 JSON 외 텍스트 제거)
            String jsonPart = extractJsonFromResponse(response);
            log.info("썸네일 생성 성공:" + jsonPart);
            return objectMapper.readValue(jsonPart, ThumbnailResult.class);
        } catch (Exception e) {
            log.warn("응답 파싱 실패, 기본값 반환: {}", response, e);
            return new ThumbnailResult(null, request.imageUrls().get(0), List.of(), request.tags(), request.title());
        }
    }
    ...
    private String buildPrompt(ThumbnailRequest request) {
        return """
    ...
    private ThumbnailResult createFallbackThumbnail(ThumbnailRequest request) {
